...
export async function createUserInfoTable(
  ...
  try {
    const results = await conn.query<RowDataPacket[]>(
      "INSERT INTO `dobru238_websocketchat`.`user_info` (`userIdInfo`, `scrollingText`, `telegramLink`, `steamLink`, `shikimoriLink`, `textStatus`, `backgroundStyle`, `colorInputWhite`, `colorInputAttention`, `colorInputNavLightBg`) VALUES (?, '', '', '', '', '', '', '#FFF', '#ffc107', '#222222')",
      myId,
    );
    return results[0];
  } catch (ex) {
